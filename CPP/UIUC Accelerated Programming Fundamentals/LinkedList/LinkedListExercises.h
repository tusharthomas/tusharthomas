...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  LinkedList<T>::Node* nodePtr = this->getHeadPtr();

  if (!nodePtr || newData <= nodePtr->data) { //if there is no head, create one
  ...
  while (nodePtr) {
    if (newData <= nodePtr->data) {
      ...
      LinkedList<T>::Node* newNode = new LinkedList<T>::Node(newData);
      LinkedList<T>::Node* prevNode = nodePtr->prev;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  LinkedList<T>::Node *leftPtr = left.getHeadPtr();
  LinkedList<T>::Node *rightPtr = right.getHeadPtr();
  ...
  while (leftPtr) {
    myData = leftPtr->data;
    while (rightPtr && rightPtr->data <= myData) {
      merged.pushBack(rightPtr->data);
      rightPtr = rightPtr->next;
    }
    merged.pushBack(myData);
