...
StringIntMap makeWordCounts(const StringVec& words) {
  ...
  for (x = 0; x < (int) words.size(); x++) {
    if (!wordcount_map.count(words[x])) {
      wordcount_map.insert(std::make_pair(words[x], 1));
    } else {
      wordcount_map[words[x]]++;
    }
  }
...
int lookupWithFallback(const StringIntMap& wordcount_map, const std::string& key, int fallbackVal) {
  ...
  if (!wordcount_map.count(key)) {
    returnValue = fallbackVal;
  } else {
    returnValue = wordcount_map.at(key);
  }
